...
#if 0
    case (char) 'б':
        if(! strcmp (".блок", name)) return COMM;
        break;
    case (char) 'г':
        if (! strcmp(".глоб", name)) return GLOBL;
        break;
    case (char) 'д':
        if (! strcmp(".данн", name)) return DATA;
        break;
    case (char) 'к':
        if (! strcmp(".код", name)) return WORD;
        if (! strcmp(".кои8", name)) return ASCII;
        break;
    case (char) 'м':
        if (! strcmp(".мблок", name)) return ACOMM;
        if (! strcmp(".мперем", name)) return ABSS;
        break;
    case (char) 'п':
        if (! strcmp(".перем", name)) return BSS;
        if (! strcmp(".полсл", name)) return HALF;
        if (! strcmp(".пам", name)) return COMM;
        break;
    case (char) 'с':
        if (! strcmp(".слово", name)) return WORD;
        if (! strcmp(".строк", name)) return STRNG;
        break;
    case (char) 'т':
        if (! strcmp(".текст", name)) return TEXT;
        break;
    case (char) 'э':
        if (! strcmp(".экв", name)) return EQU;
        break;
#endif
    }
    return -1;
